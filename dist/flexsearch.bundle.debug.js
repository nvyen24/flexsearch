...
w.assign = function(a) {
  ...
  var c = a.include, b = c || a.exclude || a.split;
  if (b || b === "") {
  ...
  this.K = null;
  if (this.matcher) {
    for (let d of this.matcher.keys()) {
      this.h += (this.h ? "|" : "") + d;
    }
  }
  if (this.stemmer) {
    for (let d of this.stemmer.keys()) {
      this.A += (this.A ? "|" : "") + d;
    }
  }
  return this;
...
w.encode = function(a, c) {
  ...
  var b = !(this.dedupe || this.mapper || this.filter || this.matcher || this.stemmer || this.replacer), e = [], d = I(), f = this.split || this.split === "" ? a.split(this.split) : [a];
  for (let l = 0, m, q; l < f.length; l++) {
    if ((m = q = f[l]) && !(m.length < this.minlength || m.length > this.maxlength)) {
      if (c) {
        if (d[m]) {
          continue;
        }
      ...
      if (b) {
        ...
        if (!this.filter || (typeof this.filter === "function" ? this.filter(m) : !this.filter.has(m))) {
          ...
          if (this.stemmer) {
            ...
            let n;
            for (; n !== m && m.length > 2;) {
              n = m, m = m.replace(this.K, t => this.stemmer.get(t));
            }
          }
          if (m && (this.mapper || this.dedupe && m.length > 1)) {
            g = "";
            for (let n = 0, t = "", u, r; n < m.length; n++) {
              u = m.charAt(n), u === t && this.dedupe || ((r = this.mapper && this.mapper.get(u)) || r === "" ? r === t && this.dedupe || !(t = r) || (g += r) : g += t = u);
            }
            m = g;
          }
          this.matcher && m.length > 1 && (this.J || (this.J = new RegExp("(" + this.h + ")", "g")), m = m.replace(this.J, n => this.matcher.get(n)));
          if (m && this.replacer) {
            for (g = 0; m && g < this.replacer.length; g += 2) {
              m = m.replace(this.replacer[g], this.replacer[g + 1]);
            }
          }
          this.cache && q.length <= this.I && (this.D.set(q, m), this.D.size > this.L && (this.D.clear(), this.I = this.I / 1.1 | 0));
          if (m) {
            if (m !== q) {
              if (c) {
                if (d[m]) {
...
var wa = {Exact:oa, Default:ra, Normalize:ra, LatinBalance:{mapper:sa}, LatinAdvanced:{mapper:sa, matcher:ta, replacer:ua}, LatinExtra:{mapper:sa, replacer:ua.concat([/(?!^)[aeo]/g, ""]), matcher:ta}, LatinSoundex:{dedupe:!1, include:{letter:!0}, finalize:function(a) {
  for (let b = 0; b < a.length; b++) {
    ...
    for (let f = 1, h; f < c.length && (h = c.charAt(f), h === "h" || h === "w" || !(h = va[h]) || h === d || (e += h, d = h, e.length !== 4)); f++) {
    }
    a[b] = e;
  }
...
function xa(a, c, b, e) {
  var d = [];
  for (let f = 0, h; f < a.index.length; f++) {
    if (h = a.index[f], c >= h.length) {
      c -= h.length;
    } else {
      c = h[e ? "splice" : "slice"](c, b);
      let g = c.length;
      if (g && (d = d.length ? d.concat(c) : c, b -= g, e && (a.length -= g), !b)) {
        break;
      }
      c = 0;
    }
...
function Aa(a) {
  ...
  return new Proxy([], {get(b, e) {
    ...
    if (e === "indexOf") {
      return function(d) {
        var f = 0;
        for (let h = 0, g, k; h < c.index.length; h++) {
          g = c.index[h];
          k = g.indexOf(d);
          if (k >= 0) {
            return f + k;
...
Q.prototype.get = function(a) {
  var c = this.index[this.B(a)];
  return c && c.get(a);
};
Q.prototype.set = function(a, c) {
  var b = this.B(a), e = this.index[b];
...
function Ba(a) {
  var c = BigInt(2) ** this.A - BigInt(1), b = typeof a;
  if (b === "bigint") {
    return a & c;
  }
  if (b === "number") {
...
function Ga(a) {
  this[a + "Async"] = function() {
    var c = arguments, b = c[c.length - 1];
    ...
    if (Ja) {
      ...
      return new Promise(h => {
        setTimeout(function() {
          h(f[a + "Async"].apply(f, c));
        }, 0);
      });
    }
    var d = this[a].apply(this, c);
...
function La(a = {}, c) {
  ...
  var e = typeof self !== "undefined" ? self._factory : typeof window !== "undefined" ? window._factory : null;
  e && (e = e.toString());
  var d = typeof window === "undefined", f = this, h = Ma(e, d, a.worker);
...
function W(a) {
  La.prototype[a] = function() {
    var c = this, b = [].slice.call(arguments), e = b[b.length - 1];
...
;Na.prototype.add = function(a, c, b) {
  ...
  if (c && (a || a === 0)) {
    ...
    for (let g = 0, k; g < this.field.length; g++) {
      k = this.B[g];
      var e = this.index.get(this.field[g]);
      ...
      for (e = 0; e < this.A.length; e++) {
        ...
        if (d && f) {
          ...
          for (let k = 0, l, m; k < f.length; k++) {
            if (l = f[k], !g[l] && (g[l] = 1, (h = d.get(l)) ? m = h : d.set(l, m = []), !b || !m.includes(a))) {
              if (m.length === 2 ** 31 - 1) {
                h = new Aa(m);
                if (this.fastupdate) {
                  for (let q of this.reg.values()) {
                    q.includes(m) && (q[q.indexOf(m)] = h);
                  }
                }
                d.set(l, m = h);
              }
              m.push(a);
              this.fastupdate && ((h = this.reg.get(a)) ? h.push(m) : this.reg.set(a, [m]));
            }
          }
        } else {
          d || console.warn("Tag '" + h + "' was not found");
        }
      }
    }
...
;function Sa(a, c, b, e) {
  ...
  for (let f = 0, h, g; f < a.length; f++) {
    if ((h = a[f]) && (g = h.length)) {
      ...
      g > c && (h = h.slice(0, c), g = c);
      if (!d.length && g >= c) {
        return e ? Ta.call(this, h) : h;
      }
      d.push(h);
      c -= g;
      if (!c) {
...
;function Ua(a, c, b, e) {
  ...
  for (c = 0; c < e.length; c++) {
    if (d = e[c]) {
      ...
      if (d.constructor === X) {
        ...
        var m = d.suggest;
        var q = d.resolve;
        var n = ((k = d.highlight || a.highlight) || d.enrich) && q;
        l = d.queue;
        let u = d.async || l, r = d.index, p = d.query;
        r ? a.index || (a.index = r) : r = a.index;
        if (p || d.tag) {
          if (!r) {
            throw Error("Resolver can't apply because the corresponding Index was never specified");
          }
          let x = d.field || d.pluck;
          if (x) {
            !p || a.query && !k || (a.query = p, a.field = x, a.highlight = k);
            if (!r.index) {
          ...
          if (l && (t || a.await)) {
            var t = 1;
            let v, A = a.C.length, E = new Promise(function(F) {
              v = F;
            });
            (function(F, B) {
              E.h = function() {
                ...
                B.resolve = !1;
                B.enrich = !1;
                var C = u ? F.searchAsync(B) : F.search(B);
                if (C.then) {
            ...
            d.resolve = !1, d.enrich = !1, d.index = null, l = u ? r.searchAsync(d) : r.search(d), d.resolve = q, d.enrich = n, d.index = r;
          }
        } else if (d.and) {
          l = Va(d, "and", r);
        } else if (d.or) {
      ...
      l.await ? (t = 1, l = l.await) : l.then ? (t = 1, l = l.then(function(u) {
        return u.result || u;
      })) : l = l.result || l;
      f[c] = l;
    }
  ...
  if (t) {
    let u = Promise.all(f).then(function(r) {
      for (let p = 0; p < a.C.length; p++) {
        if (a.C[p] === u) {
          a.C[p] = function() {
            return b.call(a, r, h, g, n, q, m, k);
          };
          break;
        }
      }
    ...
    a.C.push(function() {
      return b.call(a, f, h, g, n, q, m, k);
    });
  } else {
    return b.call(a, f, h, g, n, q, m, k);
  }
  return q ? a.await || a.result : a;
}
function Va(a, c, b) {
  a = a[c];
  var e = a[0] || a;
...
function Za(a, c, b, e, d, f, h) {
  ...
  if (a.length) {
    if (this.result.length && a.unshift(this.result), a.length < 2) {
      ...
      if (k) {
        this.result = $a(a, k, c, b, f, this.h, d);
        var g = !0;
      } else {
        this.result = [];
      }
  ...
  d && (this.await = null);
  return d ? this.resolve(c, b, e, h, g) : this;
}
;X.prototype.xor = function() {
  return Ua(this, "xor", ab, arguments);
};
function ab(a, c, b, e, d, f, h) {
  if (a.length) {
    if (this.result.length && a.unshift(this.result), a.length < 2) {
      this.result = a[0];
    } else {
      a: {
        ...
        for (let q = 0, n; q < a.length; q++) {
          if (n = a[q]) {
            ...
            for (let t = 0, u; t < n.length; t++) {
              if (u = n[t]) {
                for (let r = 0, p; r < u.length; r++) {
                  p = u[r], l[p] = l[p] ? 2 : 1;
                }
              }
            }
        ...
        for (let q = 0, n, t = 0; q < m; q++) {
          for (let u = 0, r; u < a.length; u++) {
            if (r = a[u]) {
              if (n = r[q]) {
                for (let p = 0, x; p < n.length; p++) {
                  if (x = n[p], l[x] === 1) {
                    if (f) {
                      f--;
                    } else {
...
;function cb(a, c, b, e, d) {
  ...
  h = f.substring(0, h);
  var k = d && d.boundary, l = !d || d.clip !== !1, m = d && d.merge && g && h && new RegExp(g + " " + h, "g");
  d = d && d.ellipsis;
  var q = 0;
  if (typeof d === "object") {
    var n = d.template;
  ...
  for (let Pa = 0, ea, gb, pa; Pa < c.length; Pa++) {
    ...
    ea = gb.encoder;
    r = q.get(ea);
    typeof r !== "string" && (r = ea.encode(a), q.set(ea, r));
      ...
      for (var B = 0; B < x.length; B++) {
        ...
        if (z && C) {
          ...
          for (var O = 0; O < r.length; O++) {
            ...
            P = z.indexOf(P);
            P > -1 && (G = (P ? C.substring(0, P) : "") + h + C.substring(P, P + M) + g + (P + M < D ? C.substring(P + M) : ""), L = M, y = !0);
          }
          G && (k && (A < 0 && (A = p.length + (p ? 1 : 0)), E = p.length + (p ? 1 : 0) + G.length, F += D, za.push(v.length), v.push({match:G})), p += (p ? " " : "") + G);
        }
        if (!y) {
          C = x[B], p += (p ? " " : "") + C, k && v.push({text:C});
        } else if (k && F >= k) {
          break;
        }
      }
      F = za.length * (f.length - 2);
      if (t || u || k && p.length - F > k) {
        if (F = k + F - n * 2, B = E - A, t > 0 && (B += t), u > 0 && (B += u), B <= F) {
          x = t ? A - (t > 0 ? t : 0) : A - ((F - B) / 2 | 0), v = u ? E + (u > 0 ? u : 0) : x + F, l || (x > 0 && p.charAt(x) !== " " && p.charAt(x - 1) !== " " && (x = p.indexOf(" ", x), x < 0 && (x = 0)), v < p.length && p.charAt(v - 1) !== " " && p.charAt(v) !== " " && (v = p.lastIndexOf(" ", v), v < E ? v = E : ++v)), p = (x ? d : "") + p.substring(x, v) + (v < p.length ? d : "");
        } else {
          E = [];
          A = {};
          F = {};
              ...
              if (G) {
                if (J !== G) {
                  ...
                  if (D + p.length + 1 <= k) {
                    ...
                    U = k - D - 1, U > 0 && (p = " " + p.substring(0, U), E[y] += p), B[y + 1] = 1;
                  } else {
                    B[y + 1] = 1;
                    continue;
                  }
                  ...
                  if (D + p.length + 1 <= k) {
                    ...
                    U = p.length + 1 - (k - D), U >= 0 && U < p.length && (p = p.substring(U) + " ", E[y] = p + E[y]), B[y] = 1;
                  } else {
                    B[y] = 1;
                    continue;
                  }
                ...
                y && D++;
                let Qa;
                K ? !y && n && (D += n) : (F[y] = 1, B[y] = 1);
                K >= x.length - 1 ? Qa = 1 : K < v.length - 1 && v[K + 1].match ? Qa = 1 : n && (D += n);
                D -= f.length - 2;
                if (!y || D + p.length <= k) {
                  E[y] = p;
                } else {
...
w.resolve = function(a, c, b, e, d) {
  var f = this.await ? Wa(this, !0) : this.result;
  if (f.then) {
    let h = this;
    return f.then(function() {
      return h.resolve(a, c, b, e, d);
    });
  }
...
function $a(a, c, b, e, d, f, h) {
  var g = a.length, k = [], l;
  var m = I();
  for (let q = 0, n, t, u, r; q < c; q++) {
    for (let p = 0; p < g; p++) {
      if (u = a[p], q < u.length && (n = u[q])) {
  ...
  if (a = k.length) {
    if (d) {
      k = k.length > 1 ? Ya(k, b, e, h, f) : (k = k[0]) && b && k.length > b || e ? k.slice(e, b + e) : k;
    } else {
      if (a < g) {
        return [];
      }
          ...
          for (let q = 0, n; q < k.length; q++) {
            if (n = k[q]) {
              if (e && n.length > e) {
                ...
                if (b && n.length > b || e) {
                  n = n.slice(e, b + e), b -= n.length, e && (e -= n.length);
                }
                d.push(n);
                if (!b) {
                  break;
...
function db(a, c, b, e, d) {
  ...
  if (d) {
    for (let g = 0, k; g < a.length; g++) {
      if (k = a[g], f[k]) {
        if (e) {
          e--;
        } else {
          if (h.push(k), f[k] = 0, b && --b === 0) {
            break;
          }
        }
      }
    ...
    a = a.result || a;
    for (let g = 0, k, l; g < a.length; g++) {
      for (k = a[g], c = 0; c < k.length; c++) {
        l = k[c], f[l] && ((h[g] || (h[g] = [])).push(l), f[l] = 0);
      }
    }
  }
  return h;
...
Na.prototype.search = function(a, c, b, e) {
  ...
  if (b) {
    ...
    var l = b.merge;
    var m = b.boost;
    var q = k || b.field || (q = b.index) && (q.index ? null : q);
    var n = this.tag && b.tag;
    var t = b.suggest;
    g = b.resolve !== !1;
    var u = b.cache;
    this.store && b.highlight && !g ? console.warn("Highlighting results can only be done within a resolver stage (and/or/not/xor) or when calling .resolve({ highlight: ... })") : this.store && b.enrich && !g && console.warn("Enrich results can only be done on a final resolver task or when calling .resolve({ enrich: true })");
    var r = g && this.store && b.highlight;
    var p = !!r || g && this.store && b.enrich;
    c = b.limit || c;
    var x = b.offset || 0;
    c || (c = g ? 100 : 0);
    if (n && (!this.db || !e)) {
        ...
        if (v.length) {
          for (n = 0; n < v.length; n += 2) {
            if (this.db) {
              ...
              f.push(e = e.db.tag(v[n + 1], c, x, p));
            } else {
              e = eb.call(this, v[n], v[n + 1], c, x, p);
            }
            d.push(g ? {field:v[n], tag:v[n + 1], result:e} : [e]);
          }
        }
        if (f.length) {
          let C = this;
          return Promise.all(f).then(function(z) {
            for (let D = 0; D < z.length; D++) {
              g ? d[D].result = z[D] : d[D] = z[D];
            }
            return g ? d : new X(d.length > 1 ? $a(d, 1, 0, 0, t, m) : d[0], C);
          });
        }
        return g ? d : new X(d.length > 1 ? $a(d, 1, 0, 0, t, m) : d[0], this);
      }
    }
    if (!g && !k) {
      if (q = q || this.field) {
        N(q) ? k = q : (q.constructor === Array && q.length === 1 && (q = q[0]), k = q.field || q.index);
      }
      if (!k) {
  ...
  for (let C = 0, z, D, J; C < q.length; C++) {
    D = q[C];
    if (this.db && this.tag && !this.B[C]) {
      continue;
    }
    let G;
    N(D) || (G = D, D = G.field, a = G.query || a, c = aa(G.limit, c), x = aa(G.offset, x), t = aa(G.suggest, t), r = g && this.store && aa(G.highlight, r), p = !!r || g && this.store && aa(G.enrich, p), u = aa(G.cache, u));
    if (e) {
      z = e[C];
    } else {
      A = G || b || {};
      E = A.enrich;
      var F = this.index.get(D);
      if (n) {
        if (this.db) {
          A.tag = n;
          A.field = q;
          var B = F.db.support_tag_search;
        }
        !B && E && (A.enrich = !1);
        B || (A.limit = 0, A.offset = 0);
      }
      z = u ? F.searchCache(a, n && !B ? 0 : c, A) : F.search(a, n && !B ? 0 : c, A);
      n && !B && (A.limit = c, A.offset = x);
      E && (A.enrich = E);
      if (v) {
        v[C] = z;
        continue;
    ...
    if (n && J) {
      ...
      if (this.db && e) {
        if (!B) {
          for (F = q.length; F < e.length; F++) {
            let L = e[F];
            if (L && L.length) {
              E++, A.push(L);
            } else if (!t) {
              return g ? d : new X(d, this);
            }
          }
        }
      } else {
        for (let L = 0, O, P; L < n.length; L += 2) {
          O = this.tag.get(n[L]);
          if (!O) {
            if (console.warn("Tag '" + n[L] + ":" + n[L + 1] + "' will be skipped because there is no field '" + n[L] + "'."), t) {
              continue;
            } else {
              return g ? d : new X(d, this);
            }
  ...
  if (k && (!p || !this.store)) {
    return d = d[0], g ? d : new X(d, this);
  }
  v = [];
  for (x = 0; x < f.length; x++) {
    n = d[x];
    p && n.length && typeof n[0].doc === "undefined" && (this.db ? v.push(n = this.index.get(this.field[0]).db.enrich(n)) : n = Ta.call(this, n));
    if (k) {
      return g ? r ? cb(a, n, this.index, k, r) : n : new X(n, this);
    }
  ...
  if (p && this.db && v.length) {
    ...
    return Promise.all(v).then(function(z) {
      ...
      r && (d = cb(a, d, C.index, k, r));
      return l ? fb(d) : d;
    });
  }
  r && (d = cb(a, d, this.index, k, r));
  return l ? fb(d) : d;
};
function fb(a) {
  var c = [], b = I(), e = I();
  for (let d = 0, f, h, g, k, l, m, q; d < a.length; d++) {
    f = a[d];
    h = f.field;
    g = f.result;
    for (let n = 0; n < g.length; n++) {
      if (l = g[n], typeof l !== "object" ? l = {id:k = l} : k = l.id, (m = b[k]) ? m.push(h) : (l.field = b[k] = [h], c.push(l)), q = l.highlight) {
        m = e[k], m || (e[k] = m = {}, l.highlight = m), m[h] = q;
      }
    }
  }
  return c;
...
;function Na(a) {
  ...
  if (b = c.tag) {
    if (typeof b === "string" && (b = [b]), b.length) {
      ...
      this.A = [];
      this.F = [];
      for (let d = 0, f, h; d < b.length; d++) {
        f = b[d];
        h = f.field || f;
        if (!h) {
          throw Error("The tag field from the document descriptor is undefined.");
        }
        f.custom ? this.A[d] = f.custom : (this.A[d] = hb(h, this.D), f.filter && (typeof this.A[d] === "string" && (this.A[d] = new String(this.A[d])), this.A[d].G = f.filter));
        this.F[d] = h;
        this.tag.set(h, new Map());
      }
    }
  }
  if (this.worker) {
    this.fastupdate = !1;
    a = [];
    for (let d of this.index.values()) {
      d.then && a.push(d);
    }
    if (a.length) {
      let d = this;
      return Promise.all(a).then(function(f) {
        var h = 0;
        for (let g of d.index.entries()) {
          let k = g[0], l = g[1];
          l.then && (l = f[h], d.index.set(k, l), h++);
        }
        return d;
      });
    }
...
w.mount = function(a) {
  ...
  var c = this.field;
  if (this.tag) {
    for (let f = 0, h; f < this.F.length; f++) {
      h = this.F[f];
      var b = void 0;
      this.index.set(h, b = new T({}, this.reg));
      c === this.field && (c = c.slice(0));
      c.push(h);
      b.tag = this.tag.get(h);
    }
  }
  b = [];
  var e = {db:a.db, type:a.type, fastupdate:a.fastupdate};
  for (let f = 0, h, g; f < c.length; f++) {
    e.field = g = c[f];
    h = this.index.get(g);
    let k = new a.constructor(a.id, e);
    k.id = a.id;
    b[f] = k.mount(h);
    h.document = !0;
    f ? h.bypass = !0 : h.store = this.store;
  }
  var d = this;
...
w.commit = async function() {
  var a = [];
  for (let c of this.index.values()) {
    a.push(c.commit());
  }
  await Promise.all(a);
  this.reg.clear();
...
function ib(a, c) {
  var b = new Map(), e = c.index || c.field || c;
  N(e) && (e = [e]);
  for (let f = 0, h, g; f < e.length; f++) {
    h = e[f];
    N(h) || (g = h, h = h.field);
    g = ba(g) ? Object.assign({}, a, g) : a;
    if (this.worker) {
      var d = void 0;
      d = (d = g.encoder) && d.encode ? d : new ka(typeof d === "string" ? wa[d] : d || {});
      d = new La(g, d);
      b.set(h, d);
    }
    this.worker || b.set(h, new T(g, this.reg));
    g.custom ? this.B[f] = g.custom : (this.B[f] = hb(h, this.D), g.filter && (typeof this.B[f] === "string" && (this.B[f] = new String(this.B[f])), this.B[f].G = g.filter));
    this.field[f] = h;
  }
  if (this.h) {
    a = c.store;
    N(a) && (a = [a]);
    for (let f = 0, h, g; f < a.length; f++) {
      h = a[f], g = h.field || h, h.custom ? (this.h[f] = h.custom, h.custom.O = g) : (this.h[f] = hb(g, this.D), h.filter && (typeof this.h[f] === "string" && (this.h[f] = new String(this.h[f])), this.h[f].G = h.filter));
    }
  }
  return b;
}
...
w.cleanup = function() {
  for (let a of this.index.values()) {
    a.cleanup();
  }
  return this;
};
...
function rb(a, c, b, e, d, f, h = 0) {
  var g = e && e.constructor === Array, k = g ? e.shift() : e;
  if (!k) {
    return this.export(a, c, d, f + 1);
  }
  if ((k = a((c ? c + "." : "") + (h + 1) + "." + b, JSON.stringify(k))) && k.then) {
    let l = this;
    return k.then(function() {
      return rb.call(l, a, c, b, g ? e : null, d, f, h + 1);
    });
  }
  return rb.call(this, a, c, b, g ? e : null, d, f, h + 1);
}
function jb(a, c, b = 0, e = 0) {
  if (b < this.field.length) {
    let h = this.field[b];
    if ((c = this.index.get(h).export(a, h, b, e = 1)) && c.then) {
      let g = this;
      return c.then(function() {
        return g.export(a, h, b + 1);
      });
    }
    return this.export(a, h, b + 1);
  }
  switch(e) {
    case 0:
      var d = "reg";
...
;T.prototype.remove = function(a, c) {
  var b = this.reg.size && (this.fastupdate ? this.reg.get(a) : this.reg.has(a));
  if (b) {
    if (this.fastupdate) {
...
function tb(a, c) {
  var b = 0, e = typeof c === "undefined";
  if (a.constructor === Array) {
    for (let d = 0, f, h, g; d < a.length; d++) {
      if ((f = a[d]) && f.length) {
        if (e) {
          return 1;
        }
...
T.prototype.add = function(a, c, b, e) {
  if (c && (a || a === 0)) {
    ...
    let l = c.length;
    if (l) {
      let m = I(), q = I(), n = this.resolution;
      for (let t = 0; t < l; t++) {
        let u = c[this.rtl ? l - 1 - t : t];
        var d = u.length;
        if (d && (e || !q[u])) {
          var f = this.score ? this.score(c, u, t, null, 0) : wb(n, l, t), h = "";
          switch(this.tokenize) {
            case "tolerant":
              Y(this, q, u, f, a, b);
              if (d > 2) {
                for (let r = 1, p, x, v, A; r < d - 1; r++) {
                  p = u.charAt(r), x = u.charAt(r + 1), v = u.substring(0, r) + x, A = u.substring(r + 2), h = v + p + A, Y(this, q, h, f, a, b), h = v + A, Y(this, q, h, f, a, b);
                }
                Y(this, q, u.substring(0, u.length - 1), f, a, b);
              }
              break;
            case "full":
              if (d > 2) {
                for (let r = 0, p; r < d; r++) {
                  for (f = d; f > r; f--) {
                    h = u.substring(r, f);
                    p = this.rtl ? d - 1 - r : r;
                    var g = this.score ? this.score(c, u, t, h, p) : wb(n, l, t, d, p);
                    Y(this, q, h, g, a, b);
                  }
                }
                break;
            ...
            case "reverse":
              if (d > 1) {
                for (g = d - 1; g > 0; g--) {
                  h = u[this.rtl ? d - 1 - g : g] + h;
                  var k = this.score ? this.score(c, u, t, h, g) : wb(n, l, t, d, g);
                  Y(this, q, h, k, a, b);
                }
                h = "";
              }
            case "forward":
              if (d > 1) {
                for (g = 0; g < d; g++) {
                  h += u[this.rtl ? d - 1 - g : g], Y(this, q, h, f, a, b);
                }
                break;
              }
            default:
              if (Y(this, q, u, f, a, b), e && l > 1 && t < l - 1) {
                for (d = this.N, h = u, f = Math.min(e + 1, this.rtl ? t + 1 : l - t), g = 1; g < f; g++) {
                  u = c[this.rtl ? l - 1 - t - g : t + g];
                  k = this.bidirectional && u > h;
                  let r = this.score ? this.score(c, h, t, u, g - 1) : wb(d + (l / 2 > d ? 0 : 1), l, t, f - 1, g - 1);
                  Y(this, m, k ? h : u, r, a, b, k ? u : h);
                }
              }
          }
        }
...
function Y(a, c, b, e, d, f, h) {
  ...
  if (!(g = c[b]) || h && !g[h]) {
    if (h) {
      ...
      c[h] = 1;
      var k = a.ctx;
      (g = k.get(h)) ? k = g : k.set(h, k = a.keystore ? new Q(a.keystore) : new Map());
    } else {
      k = a.map, c[b] = 1;
    }
    (g = k.get(b)) ? k = g : k.set(b, k = g = []);
    if (f) {
      for (let l = 0, m; l < g.length; l++) {
        if ((m = g[l]) && m.includes(d)) {
          if (l <= e) {
            return;
          }
...
;T.prototype.search = function(a, c, b) {
  ...
  c = c || (f ? 100 : 0);
  if (t === 1) {
    return xb.call(this, n[0], "", c, d, f, k, q);
  }
  if (t === 2 && h && !g) {
    return xb.call(this, n[1], n[0], c, d, f, k, q);
  }
  var u = I(), r = 0;
  if (h) {
    var p = n[0];
  ...
  m || m === 0 || (m = p ? this.N : this.resolution);
  if (this.db) {
    if (this.db.search && (b = this.db.search(this, n, c, d, g, f, k, q), b !== !1)) {
      return b;
    }
    let x = this;
    return async function() {
      for (let v, A; r < t; r++) {
        ...
        g && p && r === t - 1 && !e.length && (m = x.resolution, p = "", r = -1, u = I());
      }
      return Ab(e, m, c, d, g, l, f);
    }();
  }
  for (let x, v; r < t; r++) {
    if ((v = n[r]) && !u[v]) {
      u[v] = 1;
      x = yb(this, v, p, 0, 0, !1, !1);
      if (x = zb(x, e, g, m)) {
        e = x;
        break;
      }
    ...
    g && p && r === t - 1 && !e.length && (m = this.resolution, p = "", r = -1, u = I());
  }
  return Ab(e, m, c, d, g, l, f);
};
function Ab(a, c, b, e, d, f, h) {
  ...
  if (g > 1) {
    ...
    return h ? Sa.call(null, a[0], b, e) : new X(a[0], this);
  }
  return h ? k : new X(k, this);
}
function xb(a, c, b, e, d, f, h) {
  a = yb(this, a, c, b, e, d, f, h);
  return this.db ? a.then(function(g) {
    return d ? g || [] : new X(g, this);
...
function yb(a, c, b, e, d, f, h, g) {
  var k;
  b && (k = a.bidirectional && c > b) && (k = b, b = c, c = k);
  if (a.db) {
    return a.db.get(c, b, e, d, f, h, g);
  }
...
;function T(a, c) {
  ...
  b = a.context;
  var e = b === !0 ? {depth:1} : b || {}, d = N(a.encoder) ? wa[a.encoder] : a.encode || a.encoder || {};
...
w.open = function() {
  ...
  var c = Bb.open(a.id, 1);
  c.onupgradeneeded = function() {
    var b = a.db = this.result;
    for (let e = 0, d; e < Cb.length; e++) {
      d = Cb[e];
      for (let f = 0, h; f < Db[a.id].length; f++) {
        h = Db[a.id][f], b.objectStoreNames.contains(d + (d !== "reg" ? h ? ":" + h : "" : "")) || b.createObjectStore(d + (d !== "reg" ? h ? ":" + h : "" : ""));
      }
    }
  };
...
w.destroy = function() {
  var a = Bb.deleteDatabase(this.id);
  return Z(a);
};
w.clear = function() {
  var a = [];
  ...
  var c = this.db.transaction(a, "readwrite");
  for (let b = 0; b < a.length; b++) {
    ...
    if (d) {
      ...
      for (let l = 0, m; l < g.length; l++) {
        if ((m = g[l]) && m.length) {
          ...
          let q = b ? e + Math.min(m.length - e, b) : m.length;
          for (let n = e; n < q; n++) {
            k.push(m[n]);
          }
          e = 0;
...
w.enrich = function(a) {
  typeof a !== "object" && (a = [a]);
  var c = this.db.transaction("reg", "readonly").objectStore("reg"), b = [];
...
w.commit = async function(a) {
  ...
  a.reg.size && (await this.transaction("map", "readwrite", function(e) {
    for (let d of a.map) {
      let f = d[0], h = d[1];
      h.length && (e.get(f).onsuccess = function() {
        var g = this.result, k;
        if (g && g.length) {
          let l = Math.max(g.length, h.length);
    ...
    for (let d of a.ctx) {
      ...
      for (let g of h) {
        ...
        l.length && (e.get(f + ":" + k).onsuccess = function() {
          var m = this.result, q;
          if (m && m.length) {
            let n = Math.max(m.length, l.length);
            for (let t = 0, u, r; t < n; t++) {
              if ((r = l[t]) && r.length) {
                if ((u = m[t]) && u.length) {
    ...
    for (let d of a.store) {
      let f = d[0], h = d[1];
      e.put(typeof h === "object" ? JSON.stringify(h) : 1, f);
    }
  }) : a.bypass || await this.transaction("reg", "readwrite", function(e) {
    for (let d of a.reg.keys()) {
      e.put(1, d);
    }
...
function Fb(a, c, b) {
  var e = a.value, d = 0;
  for (let h = 0, g; h < e.length; h++) {
    if (g = b ? e : e[h]) {
      for (let k = 0, l, m; k < c.length; k++) {
        if (m = c[k], l = g.indexOf(m), l >= 0) {
          var f = 1;
          if (g.length > 1) {
            g.splice(l, 1);
          } else {
